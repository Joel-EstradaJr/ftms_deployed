...
const SearchableDropdown: React.FC<SearchableDropdownProps> = ({
  ...
  // Get selected option label or show custom value
  ...
  // Handle option selection (including custom)
  ...
  // Handle keyboard navigation (Enter for custom input)
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      setIsOpen(false);
      setSearchTerm('');
    }
    if (allowCustomInput && isOpen && e.key === 'Enter' && searchTerm &&
      ...
      {isOpen && menuPosition && typeof window !== 'undefined' && createPortal(
        ...
        >
          ...
          <div className="dropdown-options">
            {filteredOptions.length === 0 && allowCustomInput && searchTerm ? (
              <div
                className="dropdown-option custom-option"
                onClick={() => handleSelect(searchTerm)}
                style={{ fontStyle: 'italic', color: '#0070f3' }}
              >
              ...
              <>
                ...
                {/* Custom input option if not matching any option */}
