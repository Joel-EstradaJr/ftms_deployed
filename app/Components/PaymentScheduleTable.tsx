...
import { distributeAmount, validateDateRange } from '../utils/revenueScheduleCalculations';
...
const PaymentScheduleTable: React.FC<PaymentScheduleTableProps> = ({
  ...
  const handleAmountChange = (index: number, newAmount: number) => {
    ...
    const updatedItems = distributeAmount(totalAmount, scheduleItems, index, newAmount);
    onItemChange(updatedItems);
  };
        ...
        <table className="modal-table">
          ...
          <tbody className="modal-table-body">
            {scheduleItems.map((item, index) => {
              ...
              return (
                <tr key={item.id || index}>
                  ...
                  {mode !== 'add' && (
                    <td style={{ textAlign: 'right', textDecoration: amountChanged ? 'line-through' : 'none', color: amountChanged ? '#999' : 'inherit' }}>
                  ...
                  {/* Carried Over Amount */}
                  {totalCarriedOver > 0 && (
                    <td style={{ textAlign: 'right', color: hasCarryOver ? '#FF8C00' : 'inherit', fontWeight: hasCarryOver ? '600' : 'normal' }}>
                  ...
                  {showPaidColumn && (
                    <td style={{ textAlign: 'right', backgroundColor: item.paidAmount > 0 ? '#E8F5E9' : 'transparent' }}>
                      {formatMoney(item.paidAmount)}
                    </td>
                  )}

                  {/* Balance */}
                  <td style={{ textAlign: 'right', color: balance > 0 && item.isPastDue ? '#FF4949' : 'inherit', fontWeight: balance > 0 ? '600' : 'normal' }}>
                    {formatMoney(balance)}
                  </td>
            ...
            <tr style={{ borderTop: '2px solid var(--border-color)', fontWeight: '600', backgroundColor: 'var(--table-header-color)' }}>
              <td colSpan={mode !== 'add' ? 2 : 1} style={{ textAlign: 'right', padding: '10px' }}>
                TOTAL:
              </td>
              {mode !== 'add' && <td style={{ textAlign: 'right' }}>{formatMoney(scheduleItems.reduce((sum, item) => sum + item.originalDueAmount, 0))}</td>}
              {totalCarriedOver > 0 && <td style={{ textAlign: 'right', color: '#FF8C00' }}>{formatMoney(totalCarriedOver)}</td>}
              <td style={{ textAlign: 'right' }}>{formatMoney(totalDue)}</td>
              {showPaidColumn && <td style={{ textAlign: 'right', backgroundColor: '#E8F5E9' }}>{formatMoney(totalPaid)}</td>}
              <td style={{ textAlign: 'right', color: totalBalance > 0 ? '#FF4949' : '#4CAF50' }}>{formatMoney(totalBalance)}</td>
