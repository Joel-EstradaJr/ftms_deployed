                ...
                {(() => {
                  ...
                  if ((total === 0 || !total) && purchaseRequest.items && purchaseRequest.items.length > 0) {
                    total = purchaseRequest.items.reduce((sum, item) => {
                      // Calculate line total: qty * unit_cost
                      const qty = item.adjusted_quantity || item.quantity || 0;
                      const cost = item.supplier_item?.unit_cost || item.new_unit_cost || item.unit_cost || 0;
